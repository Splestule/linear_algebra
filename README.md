...
The project includes solver for ```Ax = b``` with the full ```A(x_p + x_n) = b``` solution, a least squares solver both by gradient descent and by ```A_transposed * Ax = A_transposed * b```, inversion of matrices, recursive determinant solver, ```PA = LU``` and other functions.
